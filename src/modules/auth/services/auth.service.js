...
export const registerUser = async (userData) => {
  ...
  const user = await User.create({ ...userData, password: hash_Password });
...
export const refreshAccessToken = async (refreshToken) => {
  ...
  const ExistToken = user.refreshToken.some(
    (rt) => rt.token === refreshToken && rt.expireAt > new Date(),
  );
...
export const logoutUser = async (refreshToken) => {
  if (!refreshToken) {
    throw createUnauthorizedError("Unauthorized. Please login.");
  }
